...
class BCD {
    ...
    static getSubstractWith9complement(bigger, smaller) {
        const complement = BCD.get9complement(smaller, bigger.length);
        return (bigger.add(complement) & (2 ** (bigger.length * 4) - 1)) + 1;
    }
    valueOf() {
        return this.numbers.reduce((acc, cur, index) => acc | ((BigInt(cur) << (BigInt(index) * 28n)) & (2n ** 28n - 1n)), 0n);
    }
    get(index) {
        if (index < 0) {
    ...
    get length() {
        ...
        let number = this.numbers.at(-1) & (2 ** 28 - 1);
        while (number > 0) {
            number = number >> 4;
            result += 1;
        }
